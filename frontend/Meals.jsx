...
const Meals = () => {
  ...
  const [currentDate, setCurrentDate] = useState(new Date()); // Data odniesienia (obecny tydzień)
  ...
  // Funkcja pomocnicza: Znajdź Poniedziałek dla danej daty
  const getMonday = (d) => {
  ...
  // Obliczamy start i koniec tygodnia do API
  const monday = getMonday(currentDate);
  const friday = new Date(monday);
  friday.setDate(monday.getDate() + 4);

  // Formatowanie daty do API (YYYY-MM-DD)
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        // Backend filtruje po zakresie dat
  ...
  // Helper do wyświetlania daty (np. "Poniedziałek, 02.12")
  const formatDayHeader = (dateObj) => {
    const options = { weekday: 'long', day: 'numeric', month: 'numeric' };
    return dateObj.toLocaleDateString('pl-PL', options);
  };

  // Generujemy 5 dni tygodnia do wyświetlenia
  const daysOfWeek = [];
  for (let i = 0; i < 5; i++) {
  ...
  // Funkcja szukająca menu dla konkretnego dnia
    ...
    <div className="meals-container">
      ...
      <div className="week-navigation-card">
        ...
        <div className="current-week-label">
          Tydzień: {formatDateAPI(monday)} - {formatDateAPI(friday)}
        </div>
